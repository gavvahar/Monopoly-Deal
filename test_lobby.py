...
def test_lobby_system():
    ...
    response = client.post("/lobby", data={"action": "create_game"}, cookies=cookies, follow_redirects=False)
    assert response.status_code == 303
    assert response.headers["location"] == "/lobby"
    ...
    response = client.post("/lobby", data={"action": "start_game"}, cookies=cookies, follow_redirects=False)
    assert response.status_code == 303
